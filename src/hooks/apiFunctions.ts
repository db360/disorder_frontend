...
export const getPagesSlugsTitles = async (): Promise<Pick<WordPressPage, "id" | "slug" | "title">[]> => {
  try {
    const { data } = await client.query<{ pages: { nodes: Pick<WordPressPage, "id" | "slug" | "title">[] } }>(
      {
